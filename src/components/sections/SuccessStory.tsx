...
export function StoryCard({ story, featured }: { story: Story; featured?: boolean }) {
  return (
    <Link
      href={story.href}
      className="beam group relative flex h-full flex-col justify-end rounded-[22px] bg-dark ring-1 ring-inset ring-ink/10"
      ...
      <div className="absolute inset-0 overflow-hidden rounded-[22px]">
        {story.image ? (
          <Image
            ...
            alt=""
            fill
            sizes={featured ? "(max-width:1024px) 100vw, 60vw" : "(max-width:1024px) 50vw, 33vw"}
          ...
          >
            ...
            <Icon
              name="truck"
              className="absolute -right-6 bottom-2 h-40 w-40 text-white/[0.06]"
            />
          </div>
        )}
